```html
<style>
/* 모든 리스트 항목을 원하는 아이콘 또는 기호로 설정 */
ul, ol {
  list-style-type: none; /* 불릿 마크를 없애고 */
}

li::before {
  content: "★"; /* 원하는 아이콘 또는 기호를 설정 */
...
1. **의도 분류 챗봇** 👨‍💻`이승현`
<ul>
    <li>**작동 조건**: USER가 채팅창에 입력한 경우</li>
    <li>**분류 기준**: 배경지식, 자유대화</li>
    <li>**이슈 및 해결과정**
        <ul><li> 중복 응답이 발생<br>➡️ langchain의 memory 기능을 이용하여 중복 응답 발생을 최소화 함.</li></ul>
    </li>    
</ul>
